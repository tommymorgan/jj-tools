...
import { Bookmark } from "./stack_detection.ts";
...
export class PullRequestManager {
  ...
  async findExistingPRs(bookmarks: Bookmark[]): Promise<Map<string, ExistingPR>> {
  ...
  async createPR(options: CreatePROptions): Promise<number> {
    const args = [
      "gh", "pr", "create",
      "--title", options.title,
      "--body", options.body,
  ...
  extractPRNumber(output: string): number | null {
