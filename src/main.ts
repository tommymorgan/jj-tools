...
import { parseArguments, showHelp, validateOptions } from "./cli.ts";
import { type PRChainInfo, PRDescriptionGenerator } from "./pr_description.ts";
import { PullRequestManager } from "./pr_manager.ts";
import { type CommandExecutor, detectStack } from "./stack_detection.ts";
	...
	try {
		...
		if (options.autoBookmark) {
			...
			if (unbookmarked.length > 0) {
				...
				for (const change of unbookmarked) {
					...
					if (!options.dryRun) {
						const autoBookmark =
							await autoBookmarkManager.createAutoBookmark(change);
		...
		for (let i = 0; i < prChain.length; i++) {
			...
			if (pr.existingPR) {
				// Update existing PR
				console.log(
					`[${prCount}/${prChain.length}] 🔄 Updating PR #${pr.existingPR.number}: ${pr.bookmark} → ${pr.base}`,
				);
				...
				if (!options.dryRun) {
					...
					if (pr.existingPR.baseRefName !== pr.base) {
						...
						console.log(
							`  📝 Updated base: ${pr.existingPR.baseRefName} → ${pr.base}`,
						);
					}
				...
				// Create new PR
				console.log(
					`[${prCount}/${prChain.length}] 🆕 Creating PR: ${pr.bookmark} → ${pr.base}`,
				);
				...
				if (!options.dryRun) {
					...
					console.log(
						`  (dry-run: would create ${isDraft ? "draft" : "ready"} PR)`,
					);
				}
			}
		}
				...
				if (pr.prNumber) {
					...
					if (viewResult.code === 0) {
						try {
							...
							originalBody = descriptionGenerator.extractOriginalBody(
								prData.body || "",
							);
						} catch {
							// Ignore parse errors
		...
		if (!options.dryRun) {
			...
			console.log("\n📊 Summary:");
			if (createdCount > 0)
				console.log(`  • Created: ${createdCount} new PR(s)`);
			if (updatedCount > 0)
