  ...
  describe("generateDescription", () => {
    it("should generate description with stack position for bottom PR", () => {
      ...
      const chain: PRChainInfo[] = [
        { bookmark: "feature-1", base: "master", prNumber: 101, isDraft: false, isReady: true },
        { bookmark: "feature-2", base: "feature-1", prNumber: 102, isDraft: true, isReady: false },
        { bookmark: "feature-3", base: "feature-2", prNumber: 103, isDraft: true, isReady: false }
      ];
    ...
    it("should include full chain visualization", () => {
      ...
      assertStringIncludes(description, "• PR #101: feature-1 → master (ready for review)");
      assertStringIncludes(description, "• **PR #102: feature-2 → feature-1 (draft)**");
      assertStringIncludes(description, "• PR #103: feature-3 → feature-2 (draft)");
    ...
    it("should highlight current PR in chain visualization", () => {
      ...
      const generator = new PRDescriptionGenerator();
      const chain: PRChainInfo[] = [
        { bookmark: "fix-1", base: "develop", prNumber: 201, isDraft: false, isReady: true },
        { bookmark: "fix-2", base: "fix-1", prNumber: 202, isDraft: false, isReady: true }
      ];
      ...
      assertStringIncludes(description, "• **PR #201: fix-1 → develop (ready for review)**");
...
Run \`npm test\``;
      ...
      assertStringIncludes(description, "This PR implements user authentication.");
...
Created with jj (Jujutsu) stack-prs`;
      ...
      // Assert
      assertEquals(originalBody.trim(), "## Original Content\nThis is the original PR description.");
    });

    it("should return full body if no metadata found", () => {
      // Arrange
      const generator = new PRDescriptionGenerator();
      const simpleDescription = "This is a simple PR description without metadata.";
  ...
  describe("formatChainItem", () => {
    it("should format chain item with PR number", () => {
      ...
      // Assert
      assertEquals(formatted, "• PR #101: feature-1 → master (ready for review)");
    });
