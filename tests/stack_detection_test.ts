...
import { detectStack, StackInfo, Bookmark } from "../src/stack_detection.ts";
...
describe("Stack Detection", () => {
  describe("detectStack", () => {
    ...
    it("should extract commit messages for each bookmark", async () => {
      ...
      // Assert
      assertEquals(stack.bookmarks[0].commitMessage, "feat: add user authentication");
    ...
    it("should handle multiple bookmarks on same commit", async () => {
      ...
      assertEquals(stack.bookmarks[0].commitHash, stack.bookmarks[1].commitHash);
    });
  });
});