...
import { Bookmark } from "../src/stack_detection.ts";
...
describe("Pull Request Manager", () => {
          ...
          if (cmd.includes("pr") && cmd.includes("list")) {
            return {
              stdout: JSON.stringify([
                { number: 101, headRefName: "feature-1", baseRefName: "master", isDraft: false },
                { number: 102, headRefName: "feature-2", baseRefName: "feature-1", isDraft: true }
              ]),
              stderr: "",
              code: 0
            };
  ...
  describe("updatePR", () => {
    it("should update PR base when needed", async () => {
      ...
      // Assert
      const baseCommand = capturedCommands.find(cmd => 
        cmd.includes("edit") && cmd.includes("--base")
      );
    ...
    it("should update PR description", async () => {
      ...
      // Assert
      const bodyCommand = capturedCommands.find(cmd => 
        cmd.includes("edit") && cmd.includes("--body")
      );
  ...
  describe("buildPRChain", () => {
    it("should build correct PR chain for stack", async () => {
      ...
      const existingPRs = new Map<string, ExistingPR>([
        ["feature-1", { number: 101, headRefName: "feature-1", baseRefName: "master", isDraft: false }]
      ]);

      // Act
      const chain = await manager.buildPRChain(bookmarks, existingPRs, "master");
  ...
  describe("extractPRNumber", () => {
    it("should extract PR number from GitHub URL", () => {
      ...
      assertEquals(
        manager.extractPRNumber("Created PR #789"),
        789
      );
    });
