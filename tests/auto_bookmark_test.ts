...
qvmssloumqwzpwuuzvntslprwpnxmuwp `,
          ...
          if (cmd.includes("show") && cmd.includes("qvmssloumqwzpwuuzvntslprwpnxmuwp")) {
            return {
      ...
      assertEquals(unbookmarked[2].changeId, "qvmssloumqwzpwuuzvntslprwpnxmuwp");
...
qvmssloumqwzpwuuzvntslprwpnxmuwp feature-auth`,
      ...
      assertEquals(unbookmarked[0].changeId, "szqzyprqmrlwvkrpmvppxsutusrvpprl");
...
def456 feature-2`,
  ...
  describe("generateBookmarkName", () => {
    it("should generate valid bookmark name from commit message", () => {
      ...
      assertEquals(
        manager.generateBookmarkName("fix(auth): resolve login bug #123", "def456"),
    ...
    it("should truncate long commit messages", () => {
      ...
      const longMessage = "feat: this is a very long commit message that should be truncated to fit within reasonable limits";
      ...
      const middlePart = bookmarkName.replace("auto/", "").replace("-abc123", "");
    ...
    it("should handle special characters in commit messages", () => {
      ...
      assertEquals(
        manager.generateBookmarkName("fix: resolve (bug) in [module]", "def456"),
  ...
  describe("createAutoBookmark", () => {
    it("should create bookmark for unbookmarked change", async () => {
      ...
      assertEquals(capturedCommand.includes("auto/add-authentication-abc123"), true);
...
auto/add-user-profile-szqzyp: szqzyprq bd5c84f0 feat: add user profile`,
      ...
      assertEquals(bookmarks.some(b => b.includes("xvrxqsnr")), false);
      assertEquals(bookmarks.some(b => b.includes("feat:")), false);
    });
  });
  ...
  describe("cleanupMergedAutoBookmarks", () => {
    it("should delete auto bookmarks for merged PRs", async () => {
      ...
      const mockExecutor: CommandExecutor = {
        exec: async (cmd: string[]) => {
          if (cmd.includes("pr") && cmd.includes("view") && cmd.includes("auto/feature-abc123")) {
            return {
          ...
          if (cmd.includes("pr") && cmd.includes("view") && cmd.includes("auto/fix-bug-def456")) {
            return {
          ...
          if (cmd.includes("pr") && cmd.includes("view") && cmd.includes("auto/update-deps-ghi789")) {
            return {
  ...
  describe("cleanupOrphanedAutoBookmarks", () => {
    it("should delete auto bookmarks not in current stack", async () => {
      ...
      const currentStackBookmarks = ["feature-1", "auto/in-stack-abc123", "feature-2"];
